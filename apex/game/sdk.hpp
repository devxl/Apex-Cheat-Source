...
__forceinline s_matrix get_view_matrix() {
	auto view_render = read<uintptr_t>(sofmainisud + OFF_VIEWRENDER);
	if (!view_render)
		return s_matrix{};
...
public:
	...
	inline c_weapon weapon() {
		...
		auto weapon_handle = read<uintptr_t>(sofmainisud + OFF_ENTITY_LIST + (weapon_pointer << 5));
		return c_weapon(weapon_handle);
	}

	inline std::string s_name() {
		auto index = read<uintptr_t>(this->address + OFF_NAME_INDEX);
		auto offsets = read<uintptr_t>((sofmainisud + OFF_NAME_LIST) + ((index - 1) << 4));
		char buf[64] = { 0 };
		sofmain1337::read_physical((PVOID)offsets, &buf, 64);
		return buf;
	}
	...
	inline int distance() {
		auto local_player = read<uintptr_t>(sofmainisud + OFF_LOCAL_PLAYER);
...
__forceinline std::string class_signifier(DWORD_PTR entity) {
	...
	char buffer[128] = { 0 };
	sofmain1337::read_physical((PVOID)class_pointer, &buffer, 64);
	return buffer;
}
